  ...
  try {
    ...
    const [rows] = await db.query(
      `SELECT id, property_name, property_type, builder, location, configuration, price,
              image_path, status, display_order, created_at, updated_at
       FROM properties ORDER BY display_order ASC, created_at DESC`,
    );
    return NextResponse.json({ properties: rows });
  } catch (error) {
    console.error("Properties GET failed:", error);
...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (!Number.isInteger(displayOrder) || displayOrder < 0) {
      return NextResponse.json({ message: "Display order must be a non-negative whole number." }, { status: 400 });
    }
    ...
    const [result] = await db.execute(
      `INSERT INTO properties
       ...
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      [propertyName, propertyType, builder || null, location, configuration || null, price, imagePath, status, displayOrder],
    );
    ...
    return NextResponse.json({ message: "Unable to create property." }, { status: 500 });
  }
}
